...
public class FilmDbStorage extends BaseRepository<Film> implements FilmStorage {
    ...
    //вынес объединение таблиц в одну константу
    private static final String JOIN_TABLES = "SELECT * " +
            "FROM films f " +
            "LEFT JOIN rating_mpa m " +
            "ON f.id = m.mpa_id " +
    ...
    private Collection<Film> getFilmsForTop(String sqlQuery, Object... args) {
        Collection<Film> films = findMany(sqlQuery, args);
        Map<Integer, Set<Genre>> genres = genreDbStorage.findAllFilmsGenres();
        Map<Integer, Collection<Integer>> likes = likeDbStorage.findAllFilmsLikes();
        for (Film film : films) {
            film.setGenres(genres.getOrDefault(film.getId(), new LinkedHashSet<>()));
